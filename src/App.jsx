...
// Onzichtbare Google Analytics helper-functie
const trackGAEvent = (eventName) => {
  if (typeof window !== 'undefined' && typeof window.gtag === 'function') {
    window.gtag('event', eventName, { 'event_category': 'Calculator' });
  }
};
...
export default function ProDakcoatingRekenApp() {
  ...
  // Helper-functie voor navigeren met ingebouwde tracker
  function handleSetStep(nextStep) {
    setStep(nextStep);
    trackGAEvent(`calculator_stap_${nextStep}`);
  }
  ...
  function downloadPdf() {
    ...
    if (!form.name || !form.email || !form.phone) {
      alert("Vul a.u.b. eerst uw naam, e-mailadres en telefoonnummer in bij Stap 5.");
      return;
    }
    ...
    doc.text(`Kleur coating: ${form.coatingColor === "wit" ? "Wit" : "RAL " + form.coatingColor}`, 20, 55);
  ...
  return (
    // HARDCODED PADDING-BOTTOM ZODAT DE TRUSTBALK NERGENS MEER OVERHEEN VALT
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 font-sans" style={{ paddingBottom: "350px" }}>
      <div className="max-w-4xl mx-auto">
        <section className="rounded-3xl bg-slate-900 border border-white/10 p-6 mb-5 shadow-xl">
          ...
          <div className="mt-6 rounded-3xl overflow-hidden border border-cyan-400/20 relative h-[260px] md:h-[340px] shadow-2xl">
            <img src={frontDakFoto} alt="Cool roof coating" className="absolute inset-0 w-full h-full object-cover" />
            ...
            <div className="relative z-10 h-full flex flex-col justify-end p-6 md:p-8">
              <div className="max-w-2xl">
                ...
                <h2 className="text-3xl md:text-5xl font-black mt-3 leading-tight">Verleng de levensduur van uw platte dak zonder vervanging</h2>
                <p className="text-slate-200 mt-4 text-base md:text-lg">Reflecterend, waterdicht en ontwikkeld voor renovatie van bitumen, EPDM en PVC daken.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <nav className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-5">
          {steps.map((label, index) => (
            <button
              ...
              className={step === index + 1 ? "rounded-2xl p-2 text-xs md:p-3 bg-cyan-400 text-slate-950 font-black transition-all" : "rounded-2xl p-2 text-xs md:p-3 bg-white/10 text-slate-300 font-bold border border-white/10 transition-all"}
            >
              {index + 1}. {label}
            </button>
          ))}
        ...
        <main className="rounded-3xl bg-slate-900 border border-white/10 p-6 md:p-8 shadow-xl">
          <div className="mb-6 flex items-center justify-between rounded-2xl bg-cyan-400 text-slate-950 px-5 py-4">
            ...
            <div className="text-right flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 bg-white/40 rounded-2xl px-3 py-2">
                <div className="w-6 h-6 rounded-full border border-black/20" style={{ backgroundColor: form.coatingColor === "wit" ? "#ffffff" : form.coatingColor === "7047" ? "#cfd3d6" : form.coatingColor === "7040" ? "#8c9297" : "#4f5358" }} />
                <div className="text-left">
                  <p className="text-xs font-bold uppercase">Kleur</p>
                  <p className="text-sm font-black">{form.coatingColor === "wit" ? "Wit" : "RAL " + form.coatingColor}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {step === 1 && (
            <div className="fade-in-stap space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {Object.entries(substrateLabels).map(([key, label]) => (
                    <button key={key} type="button" onClick={() => update("substrate", key)} className={form.substrate === key ? "rounded-2xl p-4 text-center font-black bg-cyan-400 text-slate-950 border-2 border-cyan-300 transition-all" : "rounded-2xl p-4 text-center font-bold bg-white/5 hover:bg-white/10 text-white border-2 border-white/10 transition-all"}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <div className="border-t border-white/10 pt-5">
                <span className="text-sm font-bold text-slate-300 block mb-3">Welke uitdagingen of klachten herkent u bij uw dak?</span>
                <div className="space-y-3">
                  <label className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5 cursor-pointer hover:bg-white/10 transition-all">
                    <input type="checkbox" checked={form.challengeCrack} onChange={(e) => update("challengeCrack", e.target.checked)} className="w-5 h-5 rounded accent-cyan-400" />
                    <div>
                  ...
                  <label className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5 cursor-pointer hover:bg-white/10 transition-all">
                    <input type="checkbox" checked={form.challengeWater} onChange={(e) => update("challengeWater", e.target.checked)} className="w-5 h-5 rounded accent-cyan-400" />
                    <div>
                      <p className="font-bold">Er blijft regelmatig stilstaand water op het dak liggen</p>
                      <p className="text-xs text-slate-400">Onze 2K coating is perfect bestand tegen permanente waterbelasting.</p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5 cursor-pointer hover:bg-white/10 transition-all">
                    <input type="checkbox" checked={form.challengeLeak} onChange={(e) => update("challengeLeak", e.target.checked)} className="w-5 h-5 rounded accent-cyan-400" />
                    <div>
                      <p className="font-bold">Er is momenteel sprake van lekkage(s) of zwakke plekken</p>
                      <p className="text-xs text-slate-400">Lekkages worden extra verstevigd met glasvezelbewapening voor 100% waterdichte zekerheid.</p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5 cursor-pointer hover:bg-white/10 transition-all">
                    <input type="checkbox" checked={form.challengeHeat} onChange={(e) => update("challengeHeat", e.target.checked)} className="w-5 h-5 rounded accent-cyan-400" />
                    <div>
                      <p className="font-bold">Het binnenklimaat in het gebouw wordt in de zomer veel te warm</p>
                      <p className="text-xs text-slate-400">De witte variant weerkaatst 90% van het zonlicht.</p>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div className="border-t border-white/10 pt-5">
                ...
                >
                  ...
                  <span className="text-xs text-slate-400 mt-2 max-w-md">Klik hier om direct een chat te openen.</span>
                </a>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="fade-in-stap">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <Select label="Btw type" value={form.customerType} onChange={(v) => update("customerType", v)} options={[["bedrijf", "Bedrijf / 21% btw"], ["particulier", "Particulier / 9% btw"]]} />
                <Select label="Kleur coating" value={form.coatingColor} onChange={(v) => update("coatingColor", v)} options={[["wit", "Wit - maximale reflectie"], ["7047", "RAL 7047 - Telegrijs 4"], ["7040", "RAL 7040 - Venstergrijs"], ["7042", "RAL 7042 - Verkeersgrijs A"]]} showColorCircle={true} />
              </div>
              {result.minimumAreaReached ? (
                <Grid items={[
                ...
                <div className="mt-6 rounded-3xl bg-amber-400 text-slate-950 p-6">
                  <h3 className="text-2xl font-black">Dakoppervlak kleiner dan 80 m²</h3>
                  <p className="mt-2">Voor kleinere oppervlaktes maken wij graag een maatwerkofferte.</p>
                </div>
              )}
              ...
              <div className="mt-8">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <CompareCard
                    ...
                    totalPrice={result.area > 0 ? (result.minimumAreaReached ? euro(result.excl) : "Maatwerk") : "Vul m² in"}
                    lifespan="30-60 jaar"
                    highlight
                  />
                  <CompareCard
                    ...
                    totalPrice={result.area > 0 ? (result.area >= 500 ? euro(result.oneComponentMinTotal) : `${euro(result.oneComponentMinTotal)} - ${euro(result.oneComponentMaxTotal)}`) : "Vul m² in"}
                    lifespan="20-25 jaar"
                  />
                  <CompareCard
                    ...
                    price="€75 - €160/m²"
                    totalPrice={result.area > 0 ? `${euro(result.bitumenMinTotal)} - ${euro(result.bitumenMaxTotal)}` : "Vul m² in"}
                    lifespan="20-30 jaar"
                  />
                </div>
              </div>
          ...
          {step === 3 && (
            <div className="fade-in-stap">
              <Title title="3. Temperatuurvergelijking" text="Indicatieve vergelijking: wit geeft het sterkste verkoelende effect." />
                ...
                <div className="grid md:grid-cols-2 min-h-[420px]">
                  <div className="relative overflow-hidden flex flex-col justify-between p-6">
                    ...
                    <div className="relative z-10">
                      <p className="font-black text-sm">VEROUDERD ZWART BITUMEN</p>
                      <h3 className="text-4xl font-black mt-4">Warmte geabsorbeerd</h3>
                    </div>
                    <div className="relative z-10 rounded-3xl border border-red-500/40 bg-black/80 p-5 max-w-[260px] mt-8">
                      <p className="text-xs uppercase text-slate-300">Oppervlaktetemperatuur</p>
                  ...
                  <div className="relative overflow-hidden flex flex-col justify-between p-6">
                    ...
                    <div className="relative z-10 text-right text-slate-950">
                      <p className="font-black text-sm">COOLSHIELD NEXTGEN 2K</p>
                      <h3 className="text-4xl font-black mt-4">Zonlicht teruggekaatst</h3>
                    </div>
                    <div className="relative z-10 rounded-3xl border border-cyan-400 bg-white/90 p-5 max-w-[260px] ml-auto mt-8 text-slate-950">
                      <p className="text-xs uppercase text-slate-500">Oppervlaktetemperatuur</p>
              ...
              <div className="mt-5 rounded-3xl bg-white/10 border border-white/10 p-6">
                <div className="grid md:grid-cols-3 gap-4 text-center">
                  <div><p className="text-sm font-bold text-slate-400">Oud zwart dak</p><p className="text-5xl font-black text-red-400 mt-2">{result.oldTemp}°C</p></div>
                  <div><p className="text-sm font-bold text-slate-400">Nieuw gecoat dak</p><p className="text-5xl font-black text-cyan-300 mt-2">{result.newTemp}°C</p></div>
                  <div><p className="text-sm font-bold text-slate-400">Verschil</p><p className="text-5xl font-black text-green-400 mt-2">-{result.tempDiff}°C</p></div>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="fade-in-stap space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <Select label="Airco aanwezig?" value={form.hasAirco} onChange={(v) => update("hasAirco", v)} options={[["yes", "Ja"], ["no", "Nee"]]} />
              </div>
              <Grid items={[
                ...
                ["Terugverdientijd", result.payback ? `${result.payback.toFixed(1)} jaar` : "n.v.t."],
              ]} />
            </div>
          )}

          {/* STAP 5: INSPECTIE & CONTACT */}
          {step === 5 && (
            <div className="fade-in-stap">
              <Title title="5. Inspectie &amp; onderhoud" text="Vraag eenvoudig een dakinspectie of onderhoud aan." />
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <Choice active={form.maintenance === "inspectie"} onClick={() => update("maintenance", "inspectie")} title="Dakinspectie" text="Telefonische bevestiging van inspectie en dakbeoordeling." />
                <Choice active={form.maintenance === "onderhoud"} onClick={() => update("maintenance", "onderhoud")} title="Onderhoud + reiniging" text="Jaarlijkse inspectie en reiniging met low pressure warm water." />
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <label className="block md:col-span-2">
                  ...
                  <textarea value={form.notes} onChange={(e) => update("notes", e.target.value)} rows={4} className="mt-2 w-full rounded-2xl bg-white text-slate-950 p-3 outline-none" placeholder="Bijvoorbeeld lekkage, zonnepanelen..." />
                </label>
              </div>
              <button type="button" onClick={submitForm} className="mt-5 w-full rounded-3xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-black text-xl py-5 transition-all">
                Aanvraag verzenden
              </button>
              {submitted && (
                <div className="mt-5 rounded-3xl bg-green-500/15 border border-green-400/20 p-6">
                  <h3 className="text-2xl font-black">Bedankt voor uw aanvraag.</h3>
                  <p className="text-slate-300 mt-2">Amarens van der Zwaag neemt spoedig contact met u op.</p>
                </div>
              )}
            </div>
          )}
          ...
          {step === 6 && (
            <div className="fade-in-stap">
              <Title title="6. Samenvatting" text="Overzicht van uw kostenindicatie en vergelijking." />
              <Grid items={[
                ["Totaal excl. btw", result.minimumAreaReached ? euro(result.excl) : "Maatwerk"],
              ...
              <div className="mt-6 grid md:grid-cols-2 gap-4">
                <button type="button" onClick={downloadPdf} className="rounded-3xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-black text-xl py-5 px-6 transition-all">
          ...
          {/* VASTE KNOPPEN IN HET GRIJZE BLOK */}
          <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center">
            <button type="button" disabled={step === 1} onClick={() => handleSetStep(step - 1)} className="rounded-2xl px-6 py-4 bg-white/10 hover:bg-white/20 disabled:opacity-30 font-bold transition-all">Vorige</button>
            <button type="button" disabled={step === 6} onClick={() => handleSetStep(Math.min(6, step + 1))} className="rounded-2xl px-6 py-4 bg-cyan-400 hover:bg-cyan-300 text-slate-950 disabled:opacity-30 font-black transition-all">Volgende</button>
          </div>
...
function Select({ label, value, onChange, options, showColorCircle = false }) {
  const colorMap = { wit: "#ffffff", 7047: "#cfd3d6", 7040: "#8c9297", 7042: "#4f5358" };
  return (
    <label className="block">
      <span className="text-sm font-bold text-slate-300">{label}</span>
      <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
        {options.map(([optionValue, optionLabel]) => (
          <button type="button" key={optionValue} onClick={() => onChange(optionValue)} className={value === optionValue ? "w-full flex items-center justify-between rounded-2xl bg-cyan-400 text-slate-950 p-3 border-2 border-cyan-300" : "w-full flex items-center justify-between rounded-2xl bg-white text-slate-950 p-3 border-2 border-transparent"}>
            <div className="flex items-center gap-3">
              {showColorCircle && <div className="w-7 h-7 rounded-full border border-black/20" style={{ backgroundColor: colorMap[optionValue] || "#ffffff" }} />}
              <span className="font-semibold text-left">{optionLabel}</span>
            </div>
            {value === optionValue && <span className="text-sm font-black">ACTIEF</span>}
          </button>
        ))}
      </div>
    </label>
